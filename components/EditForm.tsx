...
const EditForm = ({ route, navigation }: { route: any, navigation: any }) => {

    const { index } = route.params;
  const formData = useFormStore((state) => state.formData[index]);
  const updateFormData = useFormStore((state) => state.updateFormData);
  const [devices, setDevices] = useState(formData.devices);
  const [ssid, setSSID] = useState(formData.ssid);
  ...
  const [userPassword, setUserPassword] = useState(formData.userPassword);
  ...
  const handleSubmit = async () => {    
    ...
    userPassword,
              ...
              console.log("Data to render:", index);
                updateFormData(index, updatedFormData); // Update the form data in the store
